...
import { db } from "./db";
import { users, hospitalPatientAccess, hospitalStaff, appointments } from "@shared/schema";
...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
    ...
    try {
      ...
      if (user.role === "patient") {
        ...
        if (patient) {
          ...
          profile = { patient: { ...patient, emergencyProfile: ep } };
        }
      }
      if (user.role === "doctor") profile = { doctor: await storage.getDoctorByUserId(user.id) };
      if (user.role === "hospital") profile = { hospital: await storage.getHospitalByUserId(user.id) };
  ...
  app.patch(api.auth.updateProfile.path, authenticate, async (req: AuthRequest, res) => {
    try {
      ...
      // Update core user name and email if provided
      ...
      if (user.role === "patient") {
        const p = await storage.getPatientByUserId(user.id);
        if (p) {
          await storage.updatePatient(p.id, {
            contactNumber: input.contactNumber,
            emergencyContact: input.emergencyContact,
            dateOfBirth: input.dateOfBirth,
            gender: input.gender,
          ...
          // Also update bloodGroup in emergency profile if provided
          if (input.bloodGroup) {
            await storage.upsertEmergencyProfile({
              patientId: p.id,
        ...
        const d = await storage.getDoctorByUserId(user.id);
        if (d) {
          await storage.updateDoctor(d.id, {
            specialization: input.specialization,
            experience: input.experience,
          });
        }
      } else if (user.role === "hospital") {
        const h = await storage.getHospitalByUserId(user.id);
        if (h) {
          await storage.updateHospital(h.id, {
      ...
      let profile: any = {};
      if (updatedUser?.role === "patient") profile = { patient: await storage.getPatientByUserId(updatedUser.id) };
      if (updatedUser?.role === "doctor") profile = { doctor: await storage.getDoctorByUserId(updatedUser.id) };
      if (updatedUser?.role === "hospital") profile = { hospital: await storage.getHospitalByUserId(updatedUser.id) };
...
AI Assessment: Based on available records, patient appears to be managed appropriately. Regular follow-up recommended.`;
  ...
  app.post("/api/admin/reset-password", authenticate, async (req: AuthRequest, res) => {
    try {
      if (req.user!.role !== "admin") return res.status(403).json({ message: "Admin access only" });
      const { username, newPassword } = req.body;
      if (!username || !newPassword) return res.status(400).json({ message: "Missing data" });
