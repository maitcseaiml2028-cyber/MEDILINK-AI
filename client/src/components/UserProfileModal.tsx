...
export function UserProfileModal({ open, onOpenChange }: { open: boolean, onOpenChange: (open: boolean) => void }) {
  ...
  const [formData, setFormData] = useState({
    ...
    bloodGroup: user?.patient?.emergencyProfile?.bloodGroup || "",
    specialization: user?.doctor?.specialization || user?.hospital?.specializations || "",
    experience: user?.doctor?.experience || "",
    licenseNumber: user?.doctor?.licenseNumber || user?.hospital?.licenseNumber || "",
      ...
      <DialogContent className="sm:max-w-md w-full bg-slate-50 border border-slate-200">
        ...
        <Tabs defaultValue="profile" className="w-full mt-2">
          ...
          <TabsContent value="profile">
            <form onSubmit={handleProfileSubmit} className="space-y-4 max-h-[60vh] overflow-y-auto px-1 scrollbar-thin">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Username</label>
                  <Input value={user.username} disabled className="mt-1 bg-slate-100 cursor-not-allowed opacity-80" />
                ...
                <div className="col-span-2">
                  ...
                  <Input type="email" value={formData.email} onChange={e => setFormData(f => ({...f, email: e.target.value}))} className="mt-1" />
                </div>
                ...
                {user.role === "patient" && (
                  <>
                    <div className="col-span-1">
                      ...
                      <Input type="date" value={formData.dateOfBirth} onChange={e => setFormData(f => ({...f, dateOfBirth: e.target.value}))} className="mt-1" />
                    </div>
                    <div className="col-span-1">
                ...
                {user.role === "doctor" && (
                  <>
                    <div className="col-span-2">
                      ...
                      <Input value={formData.licenseNumber} disabled className="mt-1 bg-slate-100 cursor-not-allowed opacity-80" />
                      <p className="text-[10px] text-slate-400 mt-1">Contact admin to change license info.</p>
                    </div>
                    <div className="col-span-2">
