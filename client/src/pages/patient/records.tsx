...
export default function PatientHealthJourney() {
  ...
  const journey = useMemo(() => {
    ...
    return allEventsFinal.map((apt: any) => ({
      apt,
  ...
  const downloadVisitPDF = (item: typeof journey[0]) => {
    const { apt, visit, records: recs, prescriptions: rxs } = item;
    const doc = new jsPDF();
    doc.setFontSize(20); doc.setTextColor(20, 150, 130);
    doc.text("MediLink — Visit Summary", 20, 25);
    doc.setFontSize(11); doc.setTextColor(80);
    doc.text(`Date: ${fmt(apt.date)}  |  Time: ${apt.time}  |  Status: ${apt.status}`, 20, 33);
    doc.line(20, 37, 190, 37);
    let y = 47;
    const line = (label: string, val: string) => {
      doc.setFont("helvetica", "bold"); doc.text(label + ":", 20, y);
      doc.setFont("helvetica", "normal"); doc.text(val || "—", 70, y); y += 9;
    };
    line("Appointment Token", apt.appointmentToken || `APT-${apt.id}`);
    line("Hospital ID", `#${apt.hospitalId}`);
    if (apt.isStandaloneDay && apt.dayVisits) {
      apt.dayVisits.forEach((v: any, idx: number) => {
        line(`Check-In ${idx + 1}`, fmtTime(v.checkInTime));
        if (v.checkOutTime) line(`Check-Out ${idx + 1}`, fmtTime(v.checkOutTime));
      });
    } else if (visit) {
      line("Check-In", fmtTime(visit.checkInTime));
      line("Check-Out", fmtTime(visit.checkOutTime));
    }
    if (apt.notes) { y += 4; doc.setFont("helvetica", "bold"); doc.text("Notes:", 20, y); y += 8; doc.setFont("helvetica", "normal"); doc.splitTextToSize(apt.notes, 160).forEach((l: string) => { doc.text(l, 20, y); y += 7; }); }
    if (rxs.length) {
      y += 6; doc.setFont("helvetica", "bold"); doc.text("Prescriptions:", 20, y); y += 8;
      rxs.forEach((rx: any) => { doc.setFont("helvetica", "normal"); doc.text(`• ${rx.diagnosis || "Prescription"}: ${JSON.parse(rx.medications || "[]").join(", ")}`, 22, y); y += 8; });
    }
    if (recs.length) {
      y += 6; doc.setFont("helvetica", "bold"); doc.text("Medical Records:", 20, y); y += 8;
      recs.forEach((r: any) => { doc.setFont("helvetica", "normal"); doc.text(`• [${r.type?.toUpperCase()}] ${r.title}`, 22, y); y += 8; });
    }
    doc.save(`MediLink-Visit-${apt.date}.pdf`);
  };
    ...
    <AppLayout>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        {[
          ...
          { label: "Completed", value: Math.max(0, journey.filter((a: any) => a.apt.status === "completed").length), color: "bg-green-600 text-white" },
          { label: "Upcoming/Active", value: Math.max(0, journey.filter((a: any) => !["completed", "cancelled"].includes(a.apt.status)).length), color: "bg-teal-600 text-white" },
        ].map(s => (
      ...
      {!journey.length ? (
        ...
        <div className="relative">
          ...
          <div className="space-y-6 pl-14">
            {journey.map(({ apt, visit, records: recs, prescriptions: rxs }) => {
              const key = String(apt.id);
              const isOpen = expanded.has(key);
              const statusColor = STATUS_COLOR[apt.status] || STATUS_COLOR.pending;

              return (
                <div key={key} className="relative">
                  {/* Timeline dot */}
                  <div className={`absolute -left-9 w-8 h-8 rounded-full flex items-center justify-center border-2 border-white shadow-md ${apt.status === "completed" ? "bg-green-500" : apt.status === "checked-in" ? "bg-teal-500" : "bg-blue-400"}`}>
                    {apt.isUnmappedDay ? <FileText className="w-4 h-4 text-white" /> : apt.status === "completed" ? <LogOut className="w-4 h-4 text-white" /> : apt.status === "checked-in" ? <Stethoscope className="w-4 h-4 text-white" /> : <Calendar className="w-4 h-4 text-white" />}
                  </div>
                        ...
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-3 mb-2">
                              <span className="text-base font-bold text-slate-900">{fmt(apt.date)}</span>
                              <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold border capitalize ${statusColor}`}>{apt.status}</span>
                            </div>

                            {/* Summary row */}
                            <div className="flex flex-wrap gap-4 text-sm text-slate-500">
                              {!apt.isUnmappedDay && <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5" />{apt.time}</span>}
                              {!apt.isUnmappedDay && apt.hospitalId && <span className="flex items-center gap-1"><Building2 className="w-3.5 h-3.5" />Hospital #{apt.hospitalId}</span>}
                              {recs.length > 0 && <span className="flex items-center gap-1 text-blue-600"><FileText className="w-3.5 h-3.5" />{recs.length} record{recs.length !== 1 ? "s" : ""}</span>}
                              {rxs.length > 0 && <span className="flex items-center gap-1 text-purple-600"><Pill className="w-3.5 h-3.5" />{rxs.length} prescription{rxs.length !== 1 ? "s" : ""}</span>}
                            </div>

                            {apt.notes && <p className="text-xs text-slate-400 mt-2 italic">"{apt.notes}"</p>}
                          </div>

                          <div className="flex items-center gap-2">
                            <Button variant="outline" size="sm" onClick={e => { e.stopPropagation(); downloadVisitPDF({ apt, visit, records: recs, prescriptions: rxs }); }} className="rounded-xl text-xs h-8 border-slate-200 hover:border-teal-400 hover:text-teal-600">
                        ...
                        <div className="border-t border-slate-100 px-5 pb-5">
                          ...
                          <div className="mt-4 relative pl-6">
                            ...
                            {(!apt.isStandaloneDay && !apt.isUnmappedDay) && (
                              <Step icon={<Calendar className="w-3.5 h-3.5 text-blue-600" />} bg="bg-blue-100" label="Appointment Booked" time={fmt(apt.date) + " " + apt.time}>
                                <p className="text-xs text-slate-500">Token: <span className="font-mono">{apt.appointmentToken || `APT-${apt.id}`}</span></p>
                                {apt.notes && <p className="text-xs text-slate-500 mt-0.5">Note: {apt.notes}</p>}
                              </Step>
                            )}

                            {/* STEP 2: Check-in (For Appointments) */}
                            ...
                            {/* STANDALONE: Render multiple Check-in / Check-out steps */}
                            {apt.isStandaloneDay && apt.dayVisits && apt.dayVisits.map((v: any, idx: number) => (
                                <div key={"dvisit-" + idx} className="mb-4">
                                  <Step icon={<LogIn className="w-3.5 h-3.5 text-teal-600" />} bg="bg-teal-100" label="Checked In" time={fmtTime(v.checkInTime)}>
                                    <p className="text-xs text-slate-500">Patient arrived at hospital</p>
                                  </Step>
                                  {v.checkOutTime && (
                            ...
                            {rxs.map((rx: any, i: number) => (
                              <Step key={"rx" + i} icon={<Pill className="w-3.5 h-3.5 text-purple-600" />} bg="bg-purple-100" label={`Prescription: ${rx.diagnosis || "General"}`} time="">
                                <div className="flex flex-wrap gap-1 mt-1">
                                  {JSON.parse(rx.medications || "[]").map((m: string, j: number) => (
                            ...
                            {recs.map((rec: any, i: number) => {
                              ...
                              return (
                                <Step key={"rec" + i} icon={<Icon className={`w-3.5 h-3.5 ${style.text}`} />} bg={style.bg} label={rec.title} time={rec.type?.toUpperCase()}>
                                  ...
                                  {rec.extractedText && <p className="text-xs text-slate-400 mt-1 font-mono bg-slate-50 p-1.5 rounded">{rec.extractedText.substring(0, 120)}...</p>}
                                  {rec.fileUrl && (
                            ...
                            {!apt.isStandaloneDay && !apt.isUnmappedDay && apt.status === "completed" && (
                              <Step icon={<LogOut className="w-3.5 h-3.5 text-green-600" />} bg="bg-green-100" label="Checked Out — Visit Completed" time={visit?.checkOutTime ? fmtTime(visit.checkOutTime) : ""}>
